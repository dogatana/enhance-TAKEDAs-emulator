...
// alternative key map for NUMPAD(ten-key)
static const int key_map_alt[10][8] = {
	...
	{0x6e, 0x00, 0x69, 0x60, 0x00, 0x6d, 0x6b, 0x6a},
	...
	if(column < 10) {
		for(int i = 0; i < 8; i++) {
			if(key_stat[key_map[column][i]]
			 	|| (key_map_alt[column][i] && key_stat[key_map_alt[column][i]])) {
				stat &= ~(1 << i);
			}
		}
	}
